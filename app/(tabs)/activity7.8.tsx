...
export default function Activity7() {
  ...
  const [result, setResult] = useState(
    (global as any).activity7Data[currentIndex].exercise2
  );

  useFocusEffect(
    useCallback(() => {
      const index = (global as any).activity7DataIndex;
      setResult((global as any).activity7Data[index]?.exercise2 ?? null);
    }, [])
  );
  ...
  function saveResult(data: any) {
    ...
    (global as any).activity7Data[index].exercise2 = data.summary;
  ...
  function goNext() {
    ...
    (global as any).activity7DataIndex = 0;
    router.push('/activity7.9' as any);
  }
  ...
  return (
    <View style={[styles.container, { backgroundColor: darkMode ? '#111' : '#fff' }]}>
      ...
      <Text style={[styles.description, { color: darkMode ? '#fff' : '#111' }]}>
        Complete 100 star jumps, then place the phone on your chest and measure breathing again.
      </Text>
