...
export default function Activity7() {
  ...
  const [result, setResult] = useState(
    (global as any).activity7Data[currentIndex].rest
  );

  useFocusEffect(
    useCallback(() => {
      const index = (global as any).activity7DataIndex;
      setResult((global as any).activity7Data[index]?.rest ?? null);
    }, [])
  );
  ...
  function saveResult(data: any) {
    ...
    (global as any).activity7Data[index].rest = data.summary;
  ...
  function goNext() {
    ...
    (global as any).activity7DataIndex = 0;
    router.push('/activity7.7' as any);
  }
  ...
  return (
    <View style={[styles.container, { backgroundColor: darkMode ? '#111' : '#fff' }]}>
      ...
      <Text style={[styles.description, { color: darkMode ? '#fff' : '#111' }]}>
        Place the phone gently on your chest while resting. Start measuring, breathe normally, then stop.
      </Text>
